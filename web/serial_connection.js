/**
 * WebSerial connection with the same public API as UsbConnection.
 *
...
const SC_SYNC0 = 0x47;
const SC_SYNC1 = 0x42;
const SC_CH_COMMAND = 0x00;
const SC_CH_DATA = 0x01;
const SC_CH_STATUS = 0x02;
const SC_MAX_PAYLOAD = 64;
...
class SerialConnection {
    ...
    _feedByte(b) {
        switch (this._rxState) {
            case 'sync1':
                if (b === SC_SYNC0) this._rxState = 'sync2';
                break;
            case 'sync2':
                if (b === SC_SYNC1) this._rxState = 'channel';
                else if (b === SC_SYNC0) this._rxState = 'sync2';
            ...
            case 'lenHi':
                this._rxLen |= b << 8;
                if (this._rxLen > SC_MAX_PAYLOAD) { this._rxState = 'sync1'; break; }
                this._rxPos = 0;
    ...
    async _writeFrame(channel, payload) {
        ...
        if (payload.length > SC_MAX_PAYLOAD) throw new Error('Payload too large');
    ...
    async sendCommand(bytes) {
        ...
        await this._writeFrame(SC_CH_COMMAND, buf);
    }

    async writeBytes(data) {
        const buf = data instanceof Uint8Array ? data : new Uint8Array(data);
        await this._writeFrame(SC_CH_DATA, buf);
    }
    ...
    async setVoltage(mode) {
        ...
        const cmd = mode === '5v' ? SC_CMD.SET_VOLTAGE_5V : SC_CMD.SET_VOLTAGE_3V3;
    ...
    async setMode(mode) {
        if (!this.isConnected) return;
        await this.sendCommand(new Uint8Array([SC_CMD.SET_MODE, mode]));
    }
}
